...
int lcdPutChar(char c)
{
    ...
    // the font is 1-bit per pixel aka 8 pixels per byte and the LCD screen is 3 pixels per byte (3bits-3bits-2bits) - the code
    // calls this a 'triplet'
    ...
    uint8_t font_bytes[font_charbytes], *cp, cb, cb_multiplier_width,
        cb_multiplier_height; // font character buffer and pointer and current byte
    uint8_t triplet;
    uint8_t pixel; // this is the index of the current pixel; we used to use the 'k' loop variable but now we might already have a
                   // pixels before the loop starts
    ...
    for (int j = 0; j < font_charbytes;) {
        ...
        for (int k = 0; k < (font_width * font_multiplier_width); k++) {
            ...
            // if its not the (very beginning) and (if either (we have process 8 pixel bits) or (we have reach the end of a row of
            // the font))
