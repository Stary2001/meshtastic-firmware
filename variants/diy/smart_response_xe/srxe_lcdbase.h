...
// depending on the type of block write command, the larges required buffer will vary; for a horizontal line is the LCD_WIDTH and
// for a vertical line is it LCD_HEIGHT

#define TRIPLET_SIZE 3
#define TRIPLET_OFFSET(n)                                                                                                        \
    (((n) % TRIPLET_SIZE) ? (TRIPLET_SIZE - ((n) % TRIPLET_SIZE)) : 0) // if not on a triplet boundary, delta to next boundary
#define TRIPLET_CEILING(n)                                                                                                       \
    (((n) % TRIPLET_SIZE) ? ((n) + (TRIPLET_SIZE - ((n) % TRIPLET_SIZE)))                                                        \
                          : (n)) // if not on a triplet boundary, increase to next boundary
#define TRIPLET_FLOOR(n)                                                                                                         \
    (((n) % TRIPLET_SIZE) ? ((n) - ((n) % TRIPLET_SIZE)) : (n)) // if not on a triplet boundary, decrease to next boundary
#define TRIPLET_ROUND(n)                                                                                                         \
    (((n) % TRIPLET_SIZE) ? ((((n) % TRIPLET_SIZE) == 1) ? ((n)-1) : ((n) + 1))                                                  \
                          : (n)) // if not on a triplet boundary, round up/down to nearest boundary
...
enum { FONT1 = 0, FONT2, FONT3, FONT4 };
...
#define LCD_CORRECT_COLOR(c)                                                                                                     \
    (((c) == 0b11011011) ? 0b11111111 : (c)) // if a color is "almost" white, correct it; FYI: it's not clear if this is needed
...
const unsigned char _lcd_power_up_commands[] PROGMEM = {
    ...
    1, 0x29,                         // Display ON
    0};
const unsigned char _lcd_power_down_commands[] PROGMEM = {1,  0x28, // display off
                                                          1,  0x10, // sleep in
                                                          99, 120,  // 120ms delay
...
// Public Functions
// --------------------------------------------------------------------------------------------
void lcdFontConfig(uint8_t id, const unsigned char *data, uint8_t width, uint8_t height, uint8_t width_bytes, uint8_t char_bytes,
                   uint8_t scale);
...
void lcdFill(uint8_t ucData)
{
    ...
    _lcd_set_active_area(0, 0, LCD_WIDTH, LCD_DRIVER_HEIGHT);
    LCD_STREAM_GRABBER_SKIP(); // we do not need to attempt and send all the blanking data

    for (y = 0; y < LCD_DRIVER_HEIGHT; y++) {
...
#ifndef CUSTOM_FONTS
    lcdFontConfig(FONT1, font_6X8_P, FONT_6X8_WIDTH, FONT_6X8_HEIGHT, FONT_6X8_WIDTHBYTES, FONT_6X8_CHARBYTES,
                  FONT_DEFAULT_SCALE);
    lcdFontConfig(FONT2, font_8X14_P, FONT_8X14_WIDTH, FONT_8X14_HEIGHT, FONT_8X14_WIDTHBYTES, FONT_8X14_CHARBYTES,
                  FONT_DEFAULT_SCALE);
