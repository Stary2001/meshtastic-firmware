...
void lcdVerticalLine(int x, int y, int height, int thickness)
{
    ...
    if (thickness == 1)
        color = (fg & 0b00011100) | (bg & 0b11100011); // use middle pixel of triplet
    if (thickness == 2)
        color = (fg & 0b11111100) | (bg & 0b00000011); // use left and middle pixel of triplet
...
void lcdRectangle(int x, int y, int cx, int cy, uint8_t mode)
{
    ...
    if (y > (LCD_HEIGHT - 1))
        y = LCD_HEIGHT - 1;
    if (x + cx > (LCD_WIDTH - 1))
        cx = (LCD_WIDTH)-x; // should this have the '-1'
    if (y + cy > (LCD_HEIGHT))
        cy = (LCD_HEIGHT)-y; // should this have the '-1'
#endif
