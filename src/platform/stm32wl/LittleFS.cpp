...
int LittleFSImpl::lfs_flash_read(const struct lfs_config *c,
    ...
    //LOG_INFO("lfs_flash_read %i %i %i", block, off, size);
    return 0;
}
...
int LittleFSImpl::lfs_flash_prog(const struct lfs_config *c,
    ...
    for(size_t i = 0; i < size; i += 8) {
        //LOG_INFO("lfs_flash_prog %08x %08x", (uint32_t)(addr + i), (uintptr_t)buffer + i);
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, (uint32_t)(addr + i), *(uint64_t*)((uintptr_t)buffer + i));
        assert(status == HAL_OK);
    }
    ...
    etc
    */
   //LOG_INFO("lfs_flash_prog %i %i %i", block, off, size);
   return 0;
}
...
int LittleFSImpl::lfs_flash_erase(const struct lfs_config *c, lfs_block_t block) {
   ...
   FLASH_EraseInitTypeDef erase_config = {
        ...
        .Page = block + (128 - (me->_size / me->_blockSize)),
        .NbPages = 1
   };
   ...
   //DEBUGV("lfs_flash_erase %i (%i %i)", block, status, page_error);
   return 0;
}
